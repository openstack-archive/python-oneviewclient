...
class OneViewClientTestCase(unittest.TestCase):
    ...
    @mock.patch.object(client.Client, 'get_server_profile_template_by_uuid',
                       autospec=True)
    def test_has_physical_mac(self, server_template_mock):
        uuid = 123

        # Negative case
        profile_template_mock = models.ServerProfileTemplate()
        setattr(profile_template_mock, "macType", "Virtual")
        ...
        self.assertRaises(exceptions.OneViewInconsistentResource,
                          self.oneview_client.has_physical_mac,
                          uuid)

        # Positive case
        setattr(profile_template_mock, "macType", "Physical")
        try:
            self.oneview_client.has_physical_mac(uuid)
        except exceptions.OneViewInconsistentResource:
