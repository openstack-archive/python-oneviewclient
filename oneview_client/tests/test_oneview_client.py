...
class OneViewClientTestCase(unittest.TestCase):
    ...
    def test_power_on_server_on(self, mock_get_pstate, mock_set_pstate,
        ...
        mock_get_pstate.return_value = states.POWER_ON
        oneview_client = client.Client(self.manager_url,
                                       self.username,
                                       self.password)
    ...
    def test_power_on_server_off(self, mock_get_pstate, mock_set_pstate,
                                 mock__authenticate):
        driver_info = {"server_hardware_uri": "/any"}
        mock_get_pstate.return_value = states.POWER_OFF
        oneview_client = client.Client(self.manager_url,
                                       self.username,
                                       self.password)
        oneview_client.power_on(driver_info)
    ...
    def test_set_power_state_server_hardware(self, mock_get_node_power,
                                             ...
                                             mock__wait_for_task,
                                             mock__authenticate):
        mock_get_node_power.return_value = states.POWER_ON
    ...
    def test_check_node_port_mac_is_compatible_with_server_profile_fail(
        ...
        exc_expected_msg = (
            "Node port is not compatible with applied server"
