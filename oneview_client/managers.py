...
class ServerHardwareManager(OneViewManager):
    ...
    def fetch_related_objects(self, sh):
        egm = EnclosureGroupManager(self.oneview_client)
        shtm = ServerHardwareTypeManager(self.oneview_client)

        def fetch_enclosure_group(self):
            if sh.enclosure_group_uri:
                eg_uuid = utils.get_uuid_from_uri(sh.enclosure_group_uri)
                return egm.get(eg_uuid)

        def fetch_server_hardware_type(self):
            if sh.server_hardware_type_uri:
                sht_uuid = utils.get_uuid_from_uri(sh.server_hardware_type_uri)
                return shtm.get(sht_uuid)

        sh.__class__.enclosure_group = property(fetch_enclosure_group)
        sh.__class__.server_hardware_type = property(
            fetch_server_hardware_type
        )

        return sh

    def get(self, uuid):
        sh = super(ServerHardwareManager, self).get(uuid)
