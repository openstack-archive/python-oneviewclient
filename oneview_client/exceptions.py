...
class OneViewServerProfileTemplateError(OneViewException):
    message = ("Server Profile Template not found in the node's"
...
class OneViewHealthStatusError(OneViewException):
    message = ("There is a health status issue with an OneView"
               " Server Profile.")


class OneViewServerProfileAssignmentError(OneViewException):
    message = ("Something went wrong applying the Server Profile.")
...
class OneViewMissingValueException(OneViewException):
