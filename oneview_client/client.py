...
class Client(BaseClient):
    ...
    def create_ethernet_network(self, name, ethernet_network_type, vlan=None):
        if not (ethernet_network_type is ETHERNET_NETWORK_TYPE_UNTAGGED or
                ethernet_network_type is ETHERNET_NETWORK_TYPE_TAGGED):
            message = "Invalid value for ethernet_network_type: " +\
                "%(ethernet_network_type)s." %\
                {"ethernet_network_type": ethernet_network_type}
            raise ValueError(message)
        if ethernet_network_type is ETHERNET_NETWORK_TYPE_UNTAGGED and vlan:
            message = "When ethernet_network_type is untagged " +\
                "you must not use vlan."
            raise ValueError(message)
        if ethernet_network_type is ETHERNET_NETWORK_TYPE_TAGGED and not vlan:
