...
class Client(BaseClient):
    ...
    def power_on(self, node_info):
        ...
        return self.set_node_power_state(node_info, states.ONEVIEW_POWER_ON)
    ...
    def set_node_power_state(
        ...
        power_state_uri = (
            node_info.get('server_hardware_uri') + '/powerState')
    ...
    def get_server_hardware(self, node_info):
        uuid = utils.get_uuid_from_uri(node_info['server_hardware_uri'])
    ...
    def get_server_profile_template(self, node_info):
        uuid = utils.get_uuid_from_uri(
            node_info['server_profile_template_uri'])
    ...
    def get_boot_order(self, node_info):
        server_profile = self.get_server_profile_from_hardware(node_info)
        return server_profile.boot.get("order")

    @auditing.audit
    def set_boot_device(
        self, node_info, new_primary_boot_device, onetime=False
    ):
        ...
        self._persistent_set_boot_device(
            node_info, boot_order, new_primary_boot_device)

    @auditing.audit
    def _persistent_set_boot_device(
        self, node_info, boot_order, new_primary_boot_device
    ):
    ...
    def validate_node_server_hardware_type(self, node_info):
        node_server_hardware_type_uri = node_info.get(
    ...
    @auditing.audit
    def validate_node_server_hardware(self, node_info, node_memory, node_cpus):
    ...
    def _validate_server_profile_template_mac_type(self, uuid):
        server_profile_template = (
            self.get_server_profile_template_by_uuid(uuid)
        )
