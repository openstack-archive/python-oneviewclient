...
class Client(BaseClient):
    ...
    @auditing.audit
    def has_physical_mac(self, uuid):
        server_profile_template = self.get_server_profile_template_by_uuid(
            uuid
        )

        if server_profile_template.macType != 'Physical':
            message = (
                "The mac configured for the server profile"
                " template %s is not physical." % server_profile_template.uri
            )
            raise exceptions.OneViewInconsistentResource(message)
