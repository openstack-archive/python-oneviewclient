...
def validate_node_server_hardware_type(driver_info, node_uuid):
    ...
    if server_hardware_sht_uri != node_sht_uri:
        message = (
            ...
            " with OneView's server hardware %(server_hardware_uri)"
...
def validate_node_server_profile_template(driver_info, node_uuid):
    ...
    node_server_hardware_uri = driver_info.get('server_hardware_uri')
    ...
    spt_enclosure_group_uri = server_profile_template_json.get('enclosureGroupUri')
    
    server_hardware_json = get_server_hardware(driver_info, node_uuid)
    sh_server_hardware_type_uri = server_hardware_json.get('serverHardwareTypeUri')
    sh_enclosure_group_uri_uri = server_hardware_json.get('serverGroupUri')

    if spt_server_hardware_type_uri != sh_server_hardware_type_uri:
        message = (
            "Server profile template %(spt_uri)s serverHardwareTypeUri is"
            " inconsistent with server hardware %(server_hardware_uri)"
            " serverHardwareTypeUri. Node validation failed." %
