...
class Client(object):
    ...
    def power_on(self, node_info):
        if self.get_node_power_state(node_info) == states.POWER_ON:
            ret = states.POWER_ON
        else:
            ret = self.set_node_power_state(node_info,
                                            states.POWER_ON)
        return ret

    def power_off(self, node_info):
        if self.get_node_power_state(node_info) == states.POWER_OFF:
    ...
    def is_node_port_mac_compatible_with_server_profile(
        ...
        primary_boot_connection = server_profile_json.get('connections')[0]
        ...
        if not is_mac_address_compatible:
            message = (
                "Node port is not compatible with applied "
    ...
    def _do_request(self, url, headers, body, request_type):
        ...
        @retrying.retry(
            ...
            wait_fixed=ONE_HUNDRED_MILLISECONDS
        )
    ...
    def _wait_for_task_to_complete(self, task):
        @retrying.retry(
            ...
            wait_fixed=ONE_THOUSAND_MILLISECONDS,
