...
def check_request_status(response):
    ...
    if status in (409,):
        ...
        LOG.error("Server error when contacting OneView: ", response.json())
...
def log_insecure_connection():
    ...
    if user_cacert == '':
        ...
        if os.path.isfile(user_cacert) is False:
            ...
            LOG.info("Secure connection to OneView is disabled in the"
...
def _wait_for_task_to_complete(task):
    ...
    LOG.debug("Waiting for task %s to complete: %s%%"
              % (uri, percent_complete))
...
def check_oneview_status():
    ...
    if status_code is not None:
        if status_code == 200:
            ...
            LOG.error("Can't connect to OneView: request timed out.")
        else:
...
def power_on(driver_info):
    if get_node_power_state(driver_info) == states.POWER_ON:
        ...
        ret = set_node_power_state(driver_info, ONEVIEW_POWER_ON)
    return ret
...
def power_off(driver_info):
    if get_node_power_state(driver_info) == states.POWER_OFF:
        ...
        ret = set_node_power_state(driver_info, ONEVIEW_POWER_OFF,
                                   PRESS_AND_HOLD)
    return ret
...
def get_node_power_state(driver_info):
    ...
    if power_state is None:
        raise exceptions.OneViewResourceNotFoundError()
    elif (power_state == ONEVIEW_POWER_ON or
            power_state == states.ONEVIEW_POWERING_OFF):
        return states.POWER_ON
    elif (power_state == ONEVIEW_POWER_OFF or
            power_state == states.ONEVIEW_POWERING_ON):
        return states.POWER_OFF
    elif power_state == states.ONEVIEW_RESETTING:
        return states.REBOOT
    else:
        return states.ERROR


def set_node_power_state(driver_info, state, press_type=MOMENTARY_PRESS):
    LOG.debug(
...
def get_server_hardware(driver_info, node_uuid=''):
    ...
    server_hardware_json = prepare_and_do_request(uri=server_hardware_uri)
