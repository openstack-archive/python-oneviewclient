...
from ironic.drivers.modules.oneview import (
    driver_oneview_exceptions as exception
)
...
def log_insecure_connection():
    ...
    if user_cacert == '':
        ...
        if os.path.isfile(user_cacert) is False:
            ...
            LOG.info(_LI("Secure connection to OneView is disabled in"
...
def check_oneview_status():
    ...
    if status_code is not None:
        if status_code == 200:
            LOG.info(_LI("Connection to OneView successfully established."))
        elif status_code == 400:
            LOG.error(_LE("Can't connect to OneView: invalid credentials."))
        elif status_code == 408:
            LOG.error(_LE("Can't connect to OneView: connection timed out."))
        else:
            LOG.error(_LE("Can't connect to OneView. Status Code: %d")
                      % (status_code))
...
def verify_oneview_version():
    if is_oneview_version_compatible() is True:
        LOG.info(_LI("Oneview's REST API version is supported by the OneView "
                     "driver."))
    else:
        LOG.error(_LE("OneView's REST API version is NOT supported by the "
