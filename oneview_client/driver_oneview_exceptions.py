...
class OneViewServerProfileTemplateError(Exception):
    message = ("Server Profile Template not found into driver_info")


class OneViewServerProfileCloneError(Exception):
    message = ("Error cloning server profile")


class OneViewMaxRetriesExceededError(Exception):
    message = ("Max connection retries to OneView exceeded")
...
class OneViewServerProfileAssociatedError(Exception):
    message = ("There is no Server Profile associated to this Server"
               " Hardware")


class OneViewErrorStateSettingPowerState(Exception):
    message = ("Get Error State in OneView trying to set power state of"
               " Server Hardware")


class OneViewTaskError(Exception):
    message = ("The task for this action in OneView returnet an error state")
