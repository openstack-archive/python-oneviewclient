...
def audit(f):
    def wrapper(self, *args, **kwargs):
        method = f.__name__ + ':' + str(uuid.uuid4())
        ...
        if self.audit_enabled and (f.__name__ in self.audit_case_methods):
            _log(self, method, ret, initial_time, end_time,
...
def _log(cls, method, ret, initial_time, end_time,
         is_ironic_request, is_oneview_request):
